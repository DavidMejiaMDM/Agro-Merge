// ==========================================
// LÓGICA DE PRODUCTOS Y CARRITO - AGRO-MERGE
// ==========================================

window.addEventListener('DOMContentLoaded', function() {
    
    // ==========================================
    // 1. GESTIÓN DE SESIÓN Y MENÚ DE USUARIO
    // ==========================================
    ...
    if (sesionIniciada === 'true' && btnLogin) {
        ...
        // Estilos inyectados para el menú de perfil
        const estilosMenu = `
        <style>
          .user-menu-container { position: relative; display: inline-block; font-family: 'Segoe UI', Roboto, sans-serif; }
          .user-menu-btn { background: #f8fafc; border: 1px solid #cbd5e1; cursor: pointer; color: #1e293b; display: flex; align-items: center; gap: 10px; padding: 6px 14px 6px 6px; border-radius: 30px; transition: all 0.2s ease; }
          .user-menu-btn:hover { background: #f1f5f9; border-color: #94a3b8; }
          .user-avatar-mini { width: 32px; height: 32px; background-color: #236c2f; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 600; font-size: 0.95rem; }
          .user-name-text { font-weight: 500; font-size: 0.95rem; }
          .user-dropdown { position: absolute; top: calc(100% + 10px); right: 0; background-color: #ffffff; min-width: 220px; box-shadow: 0 10px 25px rgba(0,0,0,0.1); border-radius: 12px; border: 1px solid #e2e8f0; overflow: hidden; z-index: 1000; text-align: left; opacity: 0; visibility: hidden; transform: translateY(-10px); transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
          .user-dropdown.show { opacity: 1; visibility: visible; transform: translateY(0); }
          .dropdown-item { padding: 12px 20px; text-decoration: none; color: #334155; display: flex; align-items: center; gap: 12px; font-size: 0.95rem; font-weight: 500; transition: background 0.2s; }
          ...
          .dropdown-logout:hover { background-color: #fef2f2; color: #b91c1c; }
          .dropdown-icon { width: 18px; height: 18px; fill: currentColor; }
        </style>
        `;

        const inicial = nombreUsuario ? nombreUsuario.charAt(0).toUpperCase() : 'U';

        // Reemplazar botón de login por menú
        btnLogin.outerHTML = `
          ${estilosMenu}
        ...
        // ==========================================
        // 2. SISTEMA DE CARRITO DE COMPRAS FLOTANTE
        // ==========================================
        ...
        const carritoHTML = `
            ...
            <div id="sidebar-carrito" style="position: fixed; top: 0; right: -450px; width: 380px; max-width: 100%; height: 100vh; background-color: white; box-shadow: -4px 0 15px rgba(0,0,0,0.1); z-index: 9999; transition: right 0.4s ease-in-out; display: flex; flex-direction: column; font-family: sans-serif;">
                ...
                <div style="padding: 20px; border-top: 1px solid #eee; background: #fff;">
                    ...
                    <button onclick="window.location.href='../Carrito/carrito.html'" style="width: 100%; padding: 14px; background-color: #1e7b36; color: white; border: none; border-radius: 30px; font-weight: bold; font-size: 15px; cursor: pointer; margin-bottom: 12px;">Finalizar tu compra</button>
    ...
    // ==========================================
    // 3. SISTEMA DE FILTROS DEL MERCADO
    // ==========================================
    
    function aplicarFiltros() {
        const busqueda = document.getElementById('input-busqueda').value.toLowerCase();
        
        const mercadoRadio = document.querySelector('input[name="mercado"]:checked');
        const mercadoFiltro = mercadoRadio ? mercadoRadio.value : 'todos';
        ...
        tarjetas.forEach(card => {
            ...
            const mercadoTexto = card.querySelector('.mercado-texto').innerText.toLowerCase();
            const precio = parseInt(card.getAttribute('data-precio'));
            const mercadoId = card.getAttribute('data-mercado').toLowerCase();

            const coincideNombre = nombre.includes(busqueda) || mercadoTexto.includes(busqueda);
            const coincidePrecio = precio >= precioMin && precio <= precioMax;
            const coincideMercado = (mercadoFiltro === 'todos' || mercadoId === mercadoFiltro);

            if (coincideNombre && coincidePrecio && coincideMercado) {
    ...
    const radiosMercado = document.querySelectorAll('input[name="mercado"]');
    radiosMercado.forEach(radio => {
